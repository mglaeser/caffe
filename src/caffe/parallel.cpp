...
void NCCL<Dtype>::run(int layer) {
  ...
  if (blobs.size() > 0) {
    ...
    caffe_gpu_scal(size, (Dtype) 1.0 / comm_->nDev,
...
void NCCL<Dtype>::on_gradients_ready() {
  if (solver_->param().layer_wise_reduce()) {
    ...
    NCCL_CHECK(ncclAllReduce(diff_, diff_, static_cast<int>(size_),
                             ...
                             cudaStreamDefault));
    caffe_gpu_scal(static_cast<int>(size_), (Dtype) 1.0 / comm_->nDev, diff_);
  }
}
...
class Worker : public InternalThread {
 ...
 protected:
  void InternalThreadEntry() {
    ...
    const int start_iter = rank0_->iter();
    s->set_iter(start_iter);
    NCCL<Dtype> nccl(s);
